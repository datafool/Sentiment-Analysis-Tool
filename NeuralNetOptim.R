...
# ## Modyfing the data format so as to use in Neural Network
# 
# y <<- matrix(rep(0,m*outputLayer), nrow=m)
# y <<-ModifyY(trainingData_y, outputLayer)$y
# training_y <-ModifyY(trainingData_y, outputLayer)$training_y
# 
# 
# x <<- cBind(matrix(rep(1,m)), trainingData_X)
# train_size = .6*m;
...
# yTest <<- y[test_size:m,]
# 
# testingY <<- training_y[test_size:m,]
# trainingY <<-training_y[1:train_size,]
# 
# ### First Step Prediction
# 
# load("./Theta100HiddenUnit_stage1.Rda")
# predTrain = predict(xTrain,theta)
# index = c(seq(1,NROW(yTrain)))
# 
...
# xTrain2 = xTrain
# xTrain = xTrain[predTrain == 2,]
# 
# 
# 
# yTrain = trainingData_y[1:train_size]
# yTrain2 = yTrain
# rm(trainingData_y)
# 
# yTrain = yTrain[predTrain ==2]
# 
# yTrain[yTrain == 4] = 3
# yTrain[yTrain == 5] = 4
# 
# 
# rm(predTest, predTrain, trainingData_X, training_y)
# 
# # gc()
# trainingY = yTrain
# yTrain = matrix(rep(0, outLayer*NROW(trainingY)), nrow = NROW(trainingY), ncol = outLayer)
# for(i in 1:NROW(trainingY))
# {
#   yTrain[i, trainingY[i]] = 1
# }
